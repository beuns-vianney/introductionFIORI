sap.ui.define(
  ...
  function (Controller) {
    ...
    return Controller.extend("introtofiori.controller.View1", {
      onInit: function () {
        const oData = {
          books: [
            {
      ...
      onBooksSelectChange: function (oEvent) {
        const oBookSelected = oEvent.getParameter("selectedItem");
        const aBooksData = this.getView().getModel().getProperty("/books");

        let oBook = aBooksData.find(
          (book) => book.id === +oBookSelected.getKey()
        );
        const oJsonModel = new sap.ui.model.json.JSONModel();
        oJsonModel.setData(oBook);
        this.getView().setModel(oJsonModel, "SelectedBook");
      },
    });
  }
);
